...
// Get a mentor by ID
const getMentorService = async (id: string) => {
  if (!Types.ObjectId.isValid(id)) throw new Error("Invalid mentor ID");
  const mentor = await mentorModel.findById(id).populate("sessionHistory");
  return mentor;
};

// Update mentor
const updateMentorService = async (id: string, payload: Partial<IMentor>) => {
  if (!Types.ObjectId.isValid(id)) throw new Error("Invalid mentor ID");
...
// Delete mentor
const deleteMentorService = async (id: string) => {
  if (!Types.ObjectId.isValid(id)) throw new Error("Invalid mentor ID");
